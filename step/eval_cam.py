...
def run(args):
    ...
    #     keys = np.pad(cam_dict['keys'] + 1, (1, 0), mode='constant') # 0 is background, class start from 1...        
    #     cams = keys[cams]
    ...
    for id in dataset.ids:
        ...
        # print(cams.shape) ### for debug
        # print(np.unique(cams))
        # print(np.unique(cam_dict['keys']))
        
        # cams = np.pad(cams, ((1, 0), (0, 0), (0, 0)), mode='constant', constant_values=args.cam_eval_thres) # get image as 0,1,2,3, ... threshold as 0.15
        # keys = np.pad(cam_dict['keys'] + 1, (1, 0), mode='constant') # 0 is background, class start from 1...
